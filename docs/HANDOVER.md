...
| Issue | Detail |
|---|---|
| **Anonymous HTTP triggers** | ~~Resolved.~~ Both are gone: `HttpTestTrigger` was deleted, and `SpikeDrawingRenderTrigger` (superseded by `Shared/PdfPageRenderer.cs`) was removed 2026-09-11. Every remaining HTTP function is `AuthorizationLevel.Function`. Note for the future: `.gitignore` and `.funcignore` are **not** effective gates for a compiled isolated worker — a `.cs` file in the project directory is compiled into the assembly and registered as a live function regardless of either. Verify with `grep authLevel bin/Debug/net10.0/functions.metadata` before publishing, not by reading source. |
