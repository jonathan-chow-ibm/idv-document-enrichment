...
| **2** | **`DocumentType` enum must match `taxonomy.yaml`** | If Agent 1 returns a label the C# enum lacks, deserialization **throws** → 3 retries → error path | Adding/renaming a document type = edit **both** `taxonomy.yaml` *and* `Models/Enums.cs` — full steps in [runbook-provisioning.md, "Add a new document type"](runbook-provisioning.md#add-a-new-document-type) |
| **3** | **Provisioning script duplicates choice lists and column sets** | Change a type/field in YAML and the SharePoint Choice column or content type no longer accepts it → **the whole write-back PATCH 400s** | Update `Provision-SharePointSchema.ps1` in the same commit, **then re-run it**. Sync axes, the site-vs-library trap, and content-type drift are all covered in [runbook-provisioning.md](runbook-provisioning.md) |
...
### Add a document type, or a new content type

Moved to [runbook-provisioning.md](runbook-provisioning.md) — it covers adding a document type to an
existing content type (a code change: `taxonomy.yaml` + `Models/Enums.cs` + `$documentTypes` +
re-provision), adding a whole new content type when a new category doesn't fit the existing 5, and why
a missing `DocumentType` choice value is worse than a missing content type (one fails the whole
write-back, the other fails silently and just skips the content-type PATCH).
