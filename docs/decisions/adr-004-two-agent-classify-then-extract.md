...
- **Cost efficiency** — Agent 1 can use GPT-4o-mini (~$0.001/doc); Agent 2 uses GPT-4o with Structured Outputs but with a focused, type-specific prompt (fewer tokens than the current all-in-one prompt)
...
- Pydantic models become polymorphic (different extraction schemas per type)
...
- Two LLM calls per document instead of one (but Agent 1 is cheap; net cost may decrease)
