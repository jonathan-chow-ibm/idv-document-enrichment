---
name: system-stock-and-flow-mapping
description: "Identify and map stocks (accumulations) and flows (rates of change) within a system. Use when analyzing why queues grow, resources deplete, backlogs build, or capacity fluctuates. Use after boundary definition to understand system structure. Produces stock-and-flow maps in docs/design/."
argument-hint: "Describe the system behavior or accumulation you want to map"
---
...
- **Stock** — an accumulation that can be measured at a point in time (queue depth, headcount, trust level, technical debt)
- **Inflow** — a rate that adds to the stock over time
- **Outflow** — a rate that drains the stock over time
- **Converter** — a factor that influences a flow rate without being a stock itself (e.g., policy, skill level, automation)
...
- What determines the rate of each flow? (This leads to converters and eventually feedback loops)
...
- Does the outflow of one stock become the inflow of another? (e.g., "resolved items" flow into "completed work" stock)
- Do stock levels influence flow rates of other stocks? (e.g., high backlog increases error rate, which increases rework inflow)

These interactions are where feedback loops emerge — flag them for the `system-causal-loop-mapping` skill.
