---
name: design-ideation-workshop
description: "Run structured brainstorming sessions to generate and evaluate solution concepts. Use when transitioning from problem definition to solution exploration, when the team needs fresh ideas, or when evaluating competing approaches. Produces ideation summaries in docs/design/."
argument-hint: "Provide the problem statement or HMW question to ideate on"
---
...
- A problem frame exists in `docs/design/problem-frames/` — if not, run `design-problem-framing` first
...
**Crazy 8s:**
Generate 8 distinct ideas in rapid succession. Quantity over quality. No filtering.

**SCAMPER:**
Apply each lens to the existing process or a baseline idea:
...
**"Yes, And...":**
Build on each idea iteratively — no idea is rejected, only extended.
...
- NEVER ideate without a defined problem — run `design-problem-framing` first
...
- Flag key assumptions per concept for handoff to `design-assumption-testing`
