...
## Next Steps (Handoffs)
- **Re-plan with Correction Brief**: After completing, the calling agent may invoke `spec.plan.create` via the Agent tool.
- **Re-validate Plan**: After completing, the calling agent may invoke `spec.validate.analyze` via the Agent tool.
- **Author Superseding ADR**: After completing, the calling agent may invoke `spec.architecture.adr` via the Agent tool.
