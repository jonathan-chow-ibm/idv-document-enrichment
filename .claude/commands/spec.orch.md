...
argument-hint: "[scope=planning|full] <feature description>"
model: sonnet
---
...
## Next Steps (Handoffs)
- **Through Planning →** → run `/spec.orch`: `[scope=planning] {feature_description}`
- **Full Implementation →** → run `/spec.orch`: `[scope=full] {feature_description}`
- **spec.capture →** → run `/spec.capture`: `{feature_description}`
- **spec.plan.create →** → run `/spec.plan.create`: Create implementation plan
- **spec.plan.tasks →** → run `/spec.plan.tasks`: Break the plan into tasks
- **spec.build.implement →** → run `/spec.build.implement`: Execute implementation
